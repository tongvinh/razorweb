    ...
    public async Task<IActionResult> OnPostAsync(string returnUrl = null)
    {
      ...
      if (ModelState.IsValid)
      {
        ...
        if (result.Succeeded)
        {
          ...
          //phat sinh token xác nhận email
          var code = await _userManager.GenerateEmailConfirmationTokenAsync(user);
          code = WebEncoders.Base64UrlEncode(Encoding.UTF8.GetBytes(code));

          // https:5000/Identity/Account/ConfirmEmail?userId=xxx&code=xxx&returnUrl=xxx
          ...
          await _emailSender.SendEmailAsync(Input.Email, "Xác nhận email",
              $"Bạn đã đăng ký tài khoản trên razorweb <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>bấm vào đây để xác nhận tài khoản</a>.");
